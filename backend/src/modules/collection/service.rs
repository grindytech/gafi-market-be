...
pub async fn find_collections_by_query(
	...
	let document = cursor.try_next().await?.ok_or("Cursor collection try_next Failed")?;
	let paginated_results = document.get_array("paginatedResults")?;

	paginated_results.into_iter().for_each(|rs| {
		let collection_str = serde_json::to_string(&rs).expect("fail to parse string");

		let owner_col: NFTCollection =
			serde_json::from_str(&collection_str).expect("fail to parse NFT Collection");

		list_collections.push(owner_col.into());
	});
